...
export default function PemasukanDetail() {
  ...
  const loadTransactionData = (transactionId: string) => {
    try {
      ...
      // Ensure it's a pemasukan
      if (data.jenis !== "Pemasukan") {
        toast.error("Halaman ini hanya untuk pemasukan");
        navigate('/keuangan/pengeluaran/' + transactionId);
        return;
      }
  ...
  if (!transaction) {
    return (
      <Layout pageTitle="Memuat Detail Pemasukan...">
        <div className="container mx-auto py-6">
          <div className="flex items-center gap-2 mb-6">
            <Button variant="ghost" size="icon" onClick={() => navigate('/keuangan/arus-kas')}>
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <h1 className="text-3xl font-bold tracking-tight">Memuat Detail Pemasukan...</h1>
          </div>
  ...
  return (
    <Layout pageTitle={`Detail Pemasukan - ${transaction.kategori}`}>
      <div className="container mx-auto py-6 space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold tracking-tight">Detail Pemasukan</h1>
          </div>
        ...
        <Card className="bg-blue-50 border-blue-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-2xl flex justify-between">
              <span>Pemasukan</span>
              <span className="text-blue-600">{formattedAmount}</span>
            </CardTitle>
